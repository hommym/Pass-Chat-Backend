...
class CallService {
    ...
    async endCall(socket, cancelCallDto) {
        ...
        await Promise.all(users.map(async (user) => {
            await objects_1.database.user.update({ where: { id: user.id }, data: !isWebUser ? { onlineStatus: "online" } : { onlineStatusWeb: "online" } });
            if ((user.onlineStatus === "call" || user.onlineStatusWeb === "call") && user.id !== enderId) {
                const userConnection = chatHandler_1.chatRouterWs.sockets.get(!isWebUser ? user.connectionId : user.webConnectionId);
                if (userConnection) {
